...
export function ChatComposer({ onSend }: ChatComposerProps) {
  ...
  return (
    <div className="bg-surface px-4 py-3 sm:px-7 sm:py-4">
      <form onSubmit={handleSubmit} className="mx-auto flex max-w-[670px] items-center gap-2 rounded-xl bg-surface px-3 py-2 shadow-neo-inset focus-within:ring-4 focus-within:ring-orange/15">
        ...
        <button type="submit" aria-label="Enviar mensagem" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-orange-light text-white shadow-neo-raised-sm transition-transform hover:scale-105 active:shadow-neo-inset-sm"><ArrowUp size={16} strokeWidth={2.5} /></button>
      </form>
