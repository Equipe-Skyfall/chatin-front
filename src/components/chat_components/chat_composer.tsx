...
export function ChatComposer({ onSend, disabled = false }: ChatComposerProps) {
  ...
  return (
    <div className="border-t border-line bg-white px-4 py-3 sm:px-7 sm:py-4">
      <form onSubmit={handleSubmit} className="mx-auto flex w-full items-center gap-2 rounded-xl border border-line bg-surface px-3 py-2 focus-within:border-orange/50">
        ...
        <button type="submit" aria-label="Enviar mensagem" disabled={disabled} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-orange-light text-white transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"><ArrowUp size={16} strokeWidth={2.5} /></button>
      </form>
